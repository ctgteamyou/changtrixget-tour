...
export function splitOnLinks(html, linkRe) {
  ...
  for (let i = 0; i < hits.length; i++) {
    if (seen.has(hits[i].href)) continue;
    seen.add(hits[i].href);
    const start = hits[i].at;
    const end = i + 1 < hits.length ? hits[i + 1].at : Math.min(hits[i].end + 3500, html.length);
    cards.push({ href: hits[i].href, html: html.slice(start, end) });
  }
  return cards;
}
...
export async function scrapeListing(config, { fetchImpl = fetch } = {}) {
  const { source, sourceName, listUrls, linkRe, idRe, imageRe, makeUrl, makeTitle } = config;
  const out = [];
